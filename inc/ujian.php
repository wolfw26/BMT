                        ...
                        while($data_mapel = mysqli_fetch_array($sql_mapel)) { ?>
                            <tr>
                                ...
                                <td width="200px" align="center">
                                	<a href="?page=ujian&action=daftartopik&id_bahan_ujian=<?php echo $data_mapel['id']; ?>">Lihat Quiz</a>
                                </td>
                            </tr>
                        	<?php
                        } ?>
	            	...
	            	<table class="table table-striped table-bordered table-hover">
                        ...
                        while($data_mapel = mysqli_fetch_array($sql_mapel)) { ?>
                            <tr>
                                ...
                                <td><?php echo $data_mapel['bahan_ujian']; ?></td>
                                <td width="200px" align="center">
                                	<a href="?page=ujian&action=daftartopik&id_bahan_ujian=<?php echo $data_mapel['id']; ?>">Lihat Soal</a>
                                </td>
                            </tr>
                        	<?php
                        } ?>
										...
										<table class="table">
										    ...
										    <tbody>
										        ...
										        <tr>
										        	...
										        	<td></td>
										        	<td>
										        		<a href="?page=ujian&action=infokerjakan&id_tq=<?php echo $data_tq['id_tq']; ?>">Kerjakan Soal</a>
										        	</td>
										        </tr>
										    </tbody>
										</table>
	...
	<div class="row">
	    <div class="col-lg-12 order-1 order-lg-2">  
	        ...
	        <div class="card"> 
	        	...
	        	<div class="card-body">
	        		<?php
		            $sql_nilai = mysqli_query($db, "SELECT * FROM nilai_pilgan WHERE id_tq = '$_GET[id_tq]' AND id_peserta = '$_SESSION[peserta]'") or die ($db->error);
		            $sql_jwb = mysqli_query($db, "SELECT * FROM jawaban WHERE id_tq = '$_GET[id_tq]' AND id_peserta = '$_SESSION[peserta]'") or die ($db->error);
	        	...
	        	<div class="card-footer">
	        		<?php
					if(mysqli_num_rows($sql_nilai) > 0 || mysqli_num_rows($sql_jwb) > 0) { ?>
						<a href="?page=ujian" >Kembali</a>
						<?php
					} else {
						$sql_cek_soal_pilgan = mysqli_query($db, "SELECT * FROM soal_pilgan WHERE id_tq = '$_GET[id_tq]'") or die ($db->error);
						...
						if(mysqli_num_rows($sql_cek_soal_pilgan) > 0 || mysqli_num_rows($sql_cek_soal_essay) > 0) { ?>
							<a href="soal_2.php?id_tq=<?php echo @$_GET['id_tq']; ?>" >Mulai Mengerjakan</a> &nbsp;&nbsp;&nbsp;
						<?php
						} else { ?>
