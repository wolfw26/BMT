    ...
    <div class="row">
        <div class="panel panel-default">
            ...
            <div class="panel-body">
                <div class="table-responsive">
                    <table width="100%">
                        <?php
                        $urut = 1;
                        $sqlFoo = "SELECT tj.id_peserta, 
                            tse.id_essay, 
                            tse.pertanyaan, tj.jawaban
                            FROM soal_essay tse
                            LEFT JOIN jawaban tj 
                                ON tj.id_soal = tse.id_essay AND tj.id_peserta = $_GET[id_peserta]
            ...
            if(@$_POST['simpan_koreksi']) {
                ...
                mysqli_query($db, "INSERT INTO nilai_essay VALUES('', '$id_tq', '$_GET[id_peserta]', '$nilai_total')") or die ($db->error);
                echo "<script>window.location='?page=ujian&action=pesertakoreksi&id_tq=".$id_tq."';</script>";
            }
            ?>
        </div>
    </div>
    ...
    <div class="row">
        <div class="panel panel-default">
            ...
            if(@$_POST['simpan_koreksi']) {
                ...
                $nilai_total = $nilai / $jumlah_soal;
                mysqli_query($db, "UPDATE nilai_essay SET nilai = '$nilai_total' WHERE id = '$_GET[id_nilai]'") or die ($db->error);
                echo "<script>window.location='?page=ujian&action=pesertakoreksi&id_tq=".$id_tq."';</script>";
            }
            ?>
        </div>
